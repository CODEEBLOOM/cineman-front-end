...
const MovieReviewSection = ({ movieId, movieTitle, onReviewSummaryChange }) => {
  ...
  return (
    <section
      id="movie-reviews"
      ref={sectionRef}
      className="container mb-10 scroll-mt-24"
    >
      <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-5 py-3">
          <h2 className="text-base font-semibold text-primary">Đánh giá phim</h2>
          <div className="flex items-center gap-3 text-sm text-slate-600">
            ...
            <span>{eligibility.reviewCount} đánh giá</span>
          </div>
        </div>

        <div className="grid gap-5 px-5 py-4 lg:grid-cols-[minmax(0,1.4fr)_340px]">
          <div>
            <div className="space-y-3">
              {isLoading ? (
                    ...
                    >
                      <div className="flex gap-3">
                        {review.avatar ? (
                          ...
                          <div
                            className={`flex h-9 w-9 flex-none items-center justify-center rounded-full bg-gradient-to-br ${gradientClass} text-xs font-bold text-white`}
                          >
                ...
                <div className="rounded-md border border-dashed border-slate-300 bg-white/80 px-4 py-6 text-center">
                  <RateReviewRounded
                    sx={{ fontSize: 28 }}
            ...
            {pageNumbers.length > 1 ? (
              <div className="mt-4 flex flex-wrap items-center gap-1.5">
                <button
                  ...
                  className="rounded-md border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700 transition hover:border-primary hover:text-primary disabled:cursor-not-allowed disabled:opacity-40"
                >
                  Trước
                </button>
          ...
          <aside className="lg:sticky lg:top-0 lg:self-start">
            <div className="rounded-md border border-slate-200 bg-slate-50 p-4">
              <h3 className="text-sm font-semibold text-primary">
                {hasExistingReview ? 'Review của bạn' : 'Viết đánh giá'}
              </h3>

              {!isViewerAuthenticated ? (
                <div className="mt-3">
                  <p className="text-sm text-slate-600">
                    Đăng nhập để gửi đánh giá cho phim này.
                  </p>
                  <button
                    type="button"
                    onClick={handleRequireLogin}
                    className="mt-3 inline-flex items-center gap-1.5 rounded-md bg-primary px-4 py-2 text-xs font-semibold text-white transition hover:bg-[#083d7c]"
                  >
                ...
                <form className="mt-3 space-y-3" onSubmit={handleSubmitReview}>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm leading-6 text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-primary focus:ring-2 focus:ring-[#d6e8f7]"
                    />
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-md bg-primary px-4 py-2 text-xs font-semibold text-white transition hover:bg-[#083d7c] disabled:cursor-not-allowed disabled:opacity-60"
                    >
                    ...
                    {hasExistingReview ? (
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 rounded-md border border-red-200 bg-white px-4 py-2 text-xs font-semibold text-red-500 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-60"
                      >
                ...
                <p className="mt-3 text-sm text-slate-600">
                  Bạn chỉ có quyền xem đánh giá công khai.
                </p>
              )}
            </div>
          </aside>
